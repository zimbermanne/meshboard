...
router.post("/:id/approve", async (req, res) => {
  ...
  try {
    ...
    const nodeRes = await client.query("SELECT credit_balance FROM nodes WHERE id=$1", [post.node_id]);
    await client.query(
      `INSERT INTO sync_queue(target_node, type, payload, priority)
       VALUES($1, 'credit_update', $2, 1)`,
      [post.node_id, JSON.stringify({ node_id: post.node_id, credit_balance: nodeRes.rows[0].credit_balance })]
    );
